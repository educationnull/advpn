...
INSTALL_USER=$(cat /etc/advpn/INSTALL_USER)
PLAT=$(cat /etc/advpn/DET_PLATFORM)
NO_UFW=$(cat /etc/advpn/NO_UFW)
PORT=$(cat /etc/advpn/INSTALL_PORT)
...
echo ":::"
    ...
    echo ":::"
    # Removing advpn files
    echo "::: Removing advpn system files..."
    $SUDO rm -rf /opt/advpn &> /dev/null
    $SUDO rm -rf /etc/.advpn &> /dev/null
    $SUDO rm -rf /home/$INSTALL_USER/ovpns &> /dev/null

    $SUDO rm -rf /var/log/*advpn* &> /dev/null
    ...
    $SUDO rm -rf /etc/advpn &> /dev/null
    $SUDO rm /usr/local/bin/advpn &> /dev/null
    $SUDO rm /etc/bash_completion.d/advpn
    ...
    echo ":::"
    printf "::: Finished removing ADVPN from your system.\n"
...
while true; do
    read -rp "::: Do you wish to completely remove ADVPN configuration and installed packages from your system? (You will be prompted for each package) [y/n]: " yn
    case $yn in
