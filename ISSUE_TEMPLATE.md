...
# ADVPN Issue Template
<!-- If the install failed: can you please copy-paste the console output after running `curl install.advpn.io | bash` between the backticks -->
...
## Console output of `curl -L install.advpn.io | bash`
```
  Output Here
```
<!-- If the generation of an .ovpn file fails / the ovpns folder stays empty, please paste the output of `advpn add` or `advpn add nopass` between the backticks -->
## Console output of `advpn add` or `advpn add nopass`
```
  Output Here
```
<!-- Please paste the output of `advpn debug` between the backticks, don't forget to substitute your public IP address if you don't want the world to know it -->
## Console output of `advpn debug`
```
  Output Here
```
